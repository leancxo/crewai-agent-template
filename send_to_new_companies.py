...
def send_emails_to_new_companies():
    """Send emails to the new companies we just added."""
    
    print("🚀 SENDING EMAILS TO NEW COMPANIES")
    print("=" * 50)
    ...
    try:
        ...
        for i, company in enumerate(all_data):
            ...
            if email_status.lower() != 'sent':
                companies_to_email.append((company, i))
        
        print(f"📧 Found {len(companies_to_email)} companies ready for email")
        
        if not companies_to_email:
            print("✅ All companies have already been emailed!")
            return
        
        # Send emails to first 10 companies
        sent_count = 0
        for company, row_index in companies_to_email[:10]:
